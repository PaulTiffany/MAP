...
// Run: node demos/dzanga-cinema/selftest.mjs
...
async function setup({reduced=false,graphics=true,delayedSound=false}={}) {
  const elements={}, presets=['morning','noon','dusk'].map(hour=>eventTarget({dataset:{hour},classList:{toggle(){}},setAttribute(){}}));
  const noop=()=>{};
  ...
  const document=eventTarget({hidden:false,body:{classList:{toggle:noop}},getElementById:element,
    querySelectorAll:()=>presets,fullscreenEnabled:false});
...
console.log('PASS: clock formatting, adaptive caps, reduced motion, 30fps scheduler, pause audio synchronization, resumed frame timings, daylight completion, hidden-tab suspension, static fallback, pending audio start.');
